...
// 落葉性欄位（常綠/落葉/半落葉等）若沒有對應的欄位角色，會落到「未知欄」，
// 之後被 fallback 啟發式（純中文 2-8 字）誤判成植物名稱，必須明確排除——
// 這是「常綠」出現在索引表植物名稱欄的根因，不是隨機字串誤判。
const NON_PLANT_NAME_RE =
  /(配置圖|平面圖|剖面圖|立面圖|示意圖|大樣圖|詳圖|索引圖|植栽表|圖例|項次|備註|規格|名稱|數量|單位|小計|合計|月份|樹型|密植|株距|比例|圖號|圖名|日期|編號)|[色型]$|^(伸展|整形|飄型|圓形|橢圓形|自然形|傘形|無花|密植|常綠|落葉|半落葉|常綠性|落葉性|半落葉性)$/
